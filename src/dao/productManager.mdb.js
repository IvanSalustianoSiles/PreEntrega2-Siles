class ProductMDBManager {
  ...
  updateProductById = async (pid, latestProduct) => {
    ...
    try {
      ...
      await this.model.findByIdAndUpdate({_id: pid}, {$set: {title: title, description: description, price: +price, code: +code, stock: +stock, category: category, status: status, thumbnail: thumbnail}});
